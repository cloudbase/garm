<script lang="ts">
	...
	async function syncTools() {
		try {
			syncing = true;
			const updatedInfo = await garmApi.forceToolsSync();
			toastStore.success('Tools Sync Started', 'Agent tools sync has been triggered successfully.');
			controllerInfo = updatedInfo;
			dispatch('updated', updatedInfo);
		} catch (err) {
			toastStore.error(
				'Sync Failed',
				err instanceof Error ? err.message : 'Failed to trigger agent tools sync'
			);
		} finally {
			syncing = false;
		}
		...
		<div class="flex items-center justify-between mb-6">
			...
			<div class="flex items-center space-x-2">
				<Button
					variant="secondary"
					size="sm"
					disabled={!controllerInfo.enable_agent_tools_sync || syncing}
					on:click={syncTools}
				>
