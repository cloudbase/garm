<script lang="ts">
	...
	function initializeForm() {
		...
		agentMode = entity.agent_mode ?? false;
	}
	...
	async function handleSubmit() {
		try {
			...
			// Check if agent mode changed
			if (agentMode !== entity.agent_mode) {
		...
		<form on:submit|preventDefault={handleSubmit} class="p-6 space-y-6">
			...
			<div class="space-y-4">
				...
				<div class="flex items-center">
					...
					/>
					<label for="agentMode" class="ml-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
						Agent Mode
					</label>
